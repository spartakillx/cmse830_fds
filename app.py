...
def safe_int_slider(label: str, min_value: int, max_value: int, value: int, step: int = 1, key: Optional[str] = None) -> int:
    """
    Avoid StreamlitAPIException when min==max or invalid step; return a stable control/value.
    """
    ...
    if max_value < min_value:
        max_value = min_value
    if max_value == min_value:
        # no range – show a read-only number_input for clarity
...
def load_team_records() -> pd.DataFrame:
    ...
    df.columns = df.columns.str.strip().str.lower()
    rename = {}
    if "season" in df.columns: rename["season"] = "year"
    if "team_name" in df.columns: rename["team_name"] = "team"
    if "win%" in df.columns: rename["win%"] = "win_pct"
    df = df.rename(columns=rename)
    for c in ["year", "wins", "losses", "win_pct"]:
        if c in df.columns: df[c] = pd.to_numeric(df[c], errors="coerce")
    if "team" in df.columns: df["team"] = df["team"].str.upper()
    if "year" in df.columns: df = df[df["year"] >= MIN_YEAR]
...
def build_tables() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if not career_df.empty:
        ...
        career_df = career_df.merge(load_accolades(), on="player_name", how="left")
...
def compute_hof_index(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        out = pd.DataFrame(columns=["player_name", "hof_index"])
        return out
...
# ---------- UI TABS ----------
tabs = st.tabs(["Overview","EDA (Season & Team)","Player Explorer","Player Comparison","Team Trends","Hall of Fame Explorer"])
...
with tabs[4]:
    ...
    if not team_df.empty and set(["team","year","win_pct"]).issubset(team_df.columns):
        ...
        fig, ax = plt.subplots(figsize=(10, 4)); ax.plot(tdf["year"], tdf["win_pct"], marker="o"); ax.set_xlabel("Year"); ax.set_ylabel("Win%"); ax.set_title(f"{sel_team} – Win% over time"); st.pyplot(fig, use_container_width=True)
        if not season_df.empty:
            ssub = season_df[season_df["team"] == sel_team]
            agg_cols = {c: "sum" for c in ["pts","reb","ast"] if c in ssub.columns}
            if "games" in ssub.columns: agg_cols["games"] = "sum"
            if agg_cols:
                team_season_stats = ssub.groupby("year").agg(agg_cols).reset_index()
                st.markdown("### Team season totals"); st.dataframe(team_season_stats, use_container_width=True)
                if "pts" in team_season_stats.columns:
                    fig, ax = plt.subplots(figsize=(10, 4)); ax.plot(team_season_stats["year"], team_season_stats["pts"], marker="o"); ax.set_xlabel("Year"); ax.set_ylabel("Total Points"); ax.set_title(f"{sel_team} – Total points per season"); st.pyplot(fig, use_container_width=True)
...
with tabs[5]:
    ...
    if career_df.empty:
        ...
        with col2:
            # step chosen adaptively; avoids step=50 on a 0..0 range
