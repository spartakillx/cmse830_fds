# app.py
# ==================================================
# CMSE 830 – NBA Analytics + HoF Index + Modeling (hardened)
# Datasets (ONLY): szymonjwiak/nba-traditional, boonpalipatana/nba-season-records-from-every-year,
#                  ryanschubertds/all-nba-aba-players-bio-stats-accolades
# ==================================================
...
# --- Optional deps ---
try:  # kagglehub might be missing in hosted sandboxes
    import kagglehub  # type: ignore
    HAVE_KAGGLEHUB = True
except Exception:
    HAVE_KAGGLEHUB = False

try:  # sklearn might be missing; we fall back to NumPy models
    ...
    def mean_squared_error(y_true, y_pred): return float(np.mean((np.asarray(y_true)-np.asarray(y_pred))**2))
    def mean_absolute_error(y_true, y_pred): return float(np.mean(np.abs(np.asarray(y_true)-np.asarray(y_pred))))
    def r2_score(y_true, y_pred):
        y_true = np.asarray(y_true); y_pred = np.asarray(y_pred)
        ss_res = np.sum((y_true-y_pred)**2); ss_tot = np.sum((y_true-np.mean(y_true))**2) + 1e-12
        return float(1 - ss_res/ss_tot)
    def accuracy_score(y_true, y_pred): return float(np.mean(np.asarray(y_true)==np.asarray(y_pred)))
    ...
    def confusion_matrix(y_true, y_pred, labels=(0,1)):
        ...
        for i,lv in enumerate(labels):
            for j, pv in enumerate(labels):
                m[i,j] = int(np.sum((y_true==lv)&(y_pred==pv)))
        return m
...
with st.sidebar:
    ...
    st.write("1) Pick a tab. 2) Adjust filters. 3) Download results. Tooltips explain each control.")
    st.info(("sklearn ✅" if HAVE_SKLEARN else "sklearn ❌ fallback mode")
            + " • " + ("kagglehub ✅" if HAVE_KAGGLEHUB else "kagglehub ❌ (using local archive if present)"))
...
TEAM_NAME_TO_ABBR: Dict[str, str] = {
    ...
    "UTAH JAZZ": "UTA", "GS WARRIORS": "GSW", "SA SPURS": "SAS", "NO PELICANS": "NOP", "NO HORNETS": "NOH"
}
...
def safe_int_slider(label: str, min_value: int, max_value: int, value: int, step: int = 1, key: Optional[str] = None) -> int:
    # Prevent Streamlit slider crash when min==max
    min_value = int(min_value); max_value = int(max_value); value = int(value); step = max(1, int(step))
...
def _read_csv_from_zip(zip_path: str, name_hint: Optional[str] = None) -> Optional[pd.DataFrame]:
    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as z:
            ...
            if target:
                with z.open(target) as f: return pd.read_csv(f)
    except Exception:
        return None
    return None

def _load_kaggle_csv_any(dataset_id: str) -> pd.DataFrame:
    if not HAVE_KAGGLEHUB:
        raise RuntimeError("kagglehub not available")
    path = kagglehub.dataset_download(dataset_id)  # type: ignore
...
def load_boxscores() -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(f"Boxscores via Kaggle unavailable ({e}); trying local archives…")
        for zp in LOCAL_ZIP_GLOBS:
            df = _read_csv_from_zip(zp, "traditional") or _read_csv_from_zip(zp, "game")
            if df is not None: break
    if df is None:
        st.error("Boxscores not found. App will run with limited features.")
        return pd.DataFrame(columns=["year","player_name","team","pts","reb","ast","stl","blk","game_id"])
    ...
    if "season" in df.columns:
        ...
        df = df.dropna(subset=["year"]); df["year"]=df["year"].astype(int); df=df[df["year"]>=MIN_YEAR]
...
def load_team_records() -> pd.DataFrame:
    def _season_to_year_any(s) -> Optional[int]:
        ...
        s = str(s).strip()
        return _safe_to_int(s.split("-")[0]) if "-" in s else _safe_to_int(s)
    df = None
    try:
        df = _load_kaggle_csv_any("boonpalipatana/nba-season-records-from-every-year")
    except Exception as e:
        st.warning(f"Team records via Kaggle unavailable ({e}); trying local archives…")
        for zp in LOCAL_ZIP_GLOBS:
    ...
    loss_col = _first_existing_column(df, ["losses","l"])
    pct_col  = _first_existing_column(df, ["win_pct","win%","w/l%","win percent","win percentage"])
    work = pd.DataFrame()
    ...
    if loss_col is not None: work["losses"] = pd.to_numeric(df[loss_col], errors="coerce")
    if pct_col  is not None: work["win_pct"] = pd.to_numeric(df[pct_col],  errors="coerce")
    ...
    if "year" in work.columns:
        work=work.dropna(subset=["year"]); work["year"]=work["year"].astype(int); work=work[work["year"]>=MIN_YEAR]
    keep=[c for c in ["year","team","team_abbr","win_pct"] if c in work.columns]
    return work[keep].dropna(subset=["team_abbr"]).drop_duplicates() if keep else pd.DataFrame(columns=["year","team_abbr","win_pct"])
...
def load_accolades() -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(f"Accolades via Kaggle unavailable ({e}); trying local archives…")
        for zp in LOCAL_ZIP_GLOBS:
    ...
    if not acc_cols:
        out=df[["player_name"]].drop_duplicates()
        out["name_key"]=out["player_name"].apply(canonical_name_key)
        return out
...
def compute_hof_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    acc_cols =["mvp","finals_mvp","championships","dpoy","all_nba_first","all_nba_total","all_star",
               "all_defensive_first","all_defensive_total","roy","scoring_titles"]
    ...
    out=df.copy(); out["hof_index"]=(ranks*100).round(1)
...
def make_hof_classification(career_df: pd.DataFrame, elite_threshold: float = 85.0) -> pd.DataFrame:
    ...
    base=career_df.copy(); base["elite"]=(base["hof_index"]>=elite_threshold).astype(int)
...
class OLSResult:
    coef: np.ndarray
    intercept: float
def np_ols(X, y, l2: float = 0.0) -> OLSResult:
    X = np.asarray(X); y=np.asarray(y).reshape(-1,1)
    Xb = np.c_[np.ones((X.shape[0],1)), X]
    I = np.eye(Xb.shape[1]); I[0,0]=0.0
    beta = np.linalg.pinv(Xb.T@Xb + l2*I) @ (Xb.T @ y)
    return OLSResult(coef=beta[1:,0], intercept=float(beta[0,0]))
def np_ols_predict(res: OLSResult, X): return np.asarray(X)@res.coef + res.intercept

@dataclass
class LogisticResult:
    coef: np.ndarray
    intercept: float
def np_logistic_train(X, y, lr=0.1, epochs=3000, l2=0.0):
    X=np.asarray(X); y=np.asarray(y).reshape(-1,1)
    Xb=np.c_[np.ones((X.shape[0],1)), X]
    w=np.zeros((Xb.shape[1],1))
    for _ in range(epochs):
        z=Xb@w; p=1/(1+np.exp(-z))
        grad=Xb.T@(p-y)/len(y) + l2*np.r_[0, w[1:,0]].reshape(-1,1)
        w -= lr*grad
    return LogisticResult(coef=w[1:,0], intercept=float(w[0,0]))
def np_logistic_proba(res: LogisticResult, X):
    X=np.asarray(X); z = X@res.coef + res.intercept
    return 1/(1+np.exp(-z))
...
class GaussianNB:
    def fit(self, X, y):
        ...
        for c in self.classes_:
            Xc=X[y==c]
            self.stats_[c]=(Xc.mean(axis=0), Xc.var(axis=0)+1e-9, len(Xc)/len(X))
        return self
    def predict_proba(self, X):
        ...
        for x in X:
            pc=[]
            for c in self.classes_:
                mu,var,prior=self.stats_[c]
                logp=np.log(prior)+np.sum(-0.5*np.log(2*np.pi*var)-((x-mu)**2)/(2*var))
                pc.append(logp)
            m=max(pc); pc=np.exp(np.array(pc)-m); pc=pc/pc.sum()
            probs.append(pc)
...
with tabs[0]:
    ...
    if accolade_cols_available:
        pretty = {"mvp":"MVP","finals_mvp":"Finals MVP","championships":"Championships","dpoy":"DPOY",
    ...
    if not season_df.empty and "win_pct" in season_df.columns:
        joined = season_df["win_pct"].notna().sum(); total=len(season_df)
    ...
    if not season_df.empty:
        ...
        with ca:
            if len(numeric_cols) >= 2:
                ...
                fig, ax = plt.subplots(figsize=(7,5)); sns.heatmap(corr, cmap="coolwarm", center=0, ax=ax)
                ax.set_title(f"Correlation ({method})"); st.pyplot(fig, use_container_width=True)
        with cb:
            if numeric_cols:
                stat = st.selectbox("Distribution stat", numeric_cols, index=0)
                fig, ax = plt.subplots(figsize=(7,5)); sns.histplot(season_df[stat].dropna(), kde=True, bins=30, ax=ax)
                ax.set_title(f"Distribution of {stat}"); st.pyplot(fig, use_container_width=True)
        ...
        if "year" in season_df.columns:
            ...
            ax.plot(tmp["year"], tmp[stat_to_plot], marker="o"); ax.set_xlabel("Year"); ax.set_ylabel(stat_to_plot.upper())
            ax.set_title(f"League {stat_to_plot.upper()} totals by year"); st.pyplot(fig, use_container_width=True)
        ...
        if not plot_df.empty:
            plot_df=plot_df.dropna(); plot_df=plot_df[(plot_df["win_pct"]>0)&(plot_df["win_pct"]<=1)]
        if plot_df.empty:
            st.info("No joined Win% available to plot.")
        else:
            fig, ax = plt.subplots(figsize=(8,5)); sns.scatterplot(data=plot_df, x=stat_choice, y="win_pct", alpha=0.35, ax=ax)
            ax.set_title(f"{stat_choice} vs Team Win%"); ax.set_ylim(0,1); st.pyplot(fig, use_container_width=True)
...
with tabs[2]:
    ...
    if "player_name" in season_df.columns and not season_df.empty:
        ...
        if stat_choices:
            ...
            fig, ax = plt.subplots(figsize=(10,4))
            ax.plot(pdf["year"], pdf[stat_to_plot], marker="o"); ax.set_xlabel("Year"); ax.set_ylabel(stat_to_plot.upper())
            ax.set_title(f"{sel_player} – {stat_to_plot.upper()} over seasons"); st.pyplot(fig, use_container_width=True)
        if "pts" in pdf.columns and len(pdf) > 0:
            ...
            ax.set_title(f"{sel_player} – Points (rolling {window})"); st.pyplot(fig, use_container_width=True)
    ...
    if "player_name" in career_df.columns and len(career_df) >= 2:
        ...
        for s in comp_stats:
            ...
            if s=="avg_team_win_pct":
                f1 = f"{float(v1):.3f}" if pd.notna(v1) else "NA"; f2 = f"{float(v2):.3f}" if pd.notna(v2) else "NA"
            elif s=="hof_index":
                f1 = f"{float(v1):.1f}" if pd.notna(v1) else "NA"; f2 = f"{float(v2):.1f}" if pd.notna(v2) else "NA"
            else:
                f1 = f"{int(v1):,}" if pd.notna(v1) else "NA"; f2 = f"{int(v2):,}" if pd.notna(v2) else "NA"
...
with tabs[4]:
    ...
    if team_season_df.empty:
        ...
        default_feats = [c for c in reg_feats_all if c.endswith("_per_game")] or reg_feats_all[:min(5,len(reg_feats_all))]
        ...
        if reg_feats:
            ...
            if HAVE_SKLEARN:
                ...
                with colA:
                    st.markdown("Linear Regression"); st.json(m_lr)
                ...
                with colB:
                    st.markdown("Random Forest Regressor"); st.json(m_rf)
                    kf = KFold(n_splits=cv_k, shuffle=True, random_state=seed)
                    cv_r2 = cross_val_score(rfr, X, y, cv=kf, scoring="r2", n_jobs=-1)
                    st.caption(f"CV R2 mean={cv_r2.mean():.3f} ± {cv_r2.std():.3f}")
                ...
                try:
                    perm = permutation_importance(rfr, X, y, n_repeats=8, random_state=seed, n_jobs=-1)
                ...
                # NumPy fallback: OLS + Ridge
                n = len(X); idx = np.random.default_rng(seed).permutation(n)
                n_test = max(1, int(test_size*n)); test_idx = idx[:n_test]; train_idx = idx[n_test:]
                X_train, X_test, y_train, y_test = X[train_idx], X[test_idx], y[train_idx], y[test_idx]
                ols = np_ols(X_train, y_train, l2=0.0)
                ridge = np_ols(X_train, y_train, l2=1.0)
                y_pred_ols = np_ols_predict(ols, X_test)
                y_pred_ridge = np_ols_predict(ridge, X_test)
                m_lr = {"RMSE": float(np.sqrt(mean_squared_error(y_test,y_pred_ols))),
                        "MAE": float(mean_absolute_error(y_test,y_pred_ols)),
                        "R2":  float(r2_score(y_test,y_pred_ols))}
                m_rg = {"RMSE": float(np.sqrt(mean_squared_error(y_test,y_pred_ridge))),
                        "MAE": float(mean_absolute_error(y_test,y_pred_ridge)),
                        "R2":  float(r2_score(y_test,y_pred_ridge))}
                colA,colB = st.columns(2)
                with colA: st.markdown("OLS (NumPy)"); st.json(m_lr)
                with colB: st.markdown("Ridge (λ=1) (NumPy)"); st.json(m_rg)
                fig, ax = plt.subplots(figsize=(7,4))
                ax.scatter(y_test, y_test-y_pred_ols, alpha=0.4, label="OLS")
                ax.scatter(y_test, y_test-y_pred_ridge, alpha=0.4, label="Ridge")
                ax.axhline(0, color="gray", linestyle="--", alpha=0.5)
                ax.set_xlabel("True Win%"); ax.set_ylabel("Residual"); ax.set_title("Residuals (test)"); ax.legend()
                st.pyplot(fig, use_container_width=True)
                pred_df = pd.DataFrame({"y_true": y_test, "y_pred_ols": y_pred_ols, "y_pred_ridge": y_pred_ridge})
                st.download_button("Download regression predictions (NumPy)", data=to_csv_bytes(pred_df), file_name="regression_predictions_numpy.csv", mime="text/csv")
    ...
    if hof_clf_df.empty:
        ...
        if class_feats:
            X = clf_df[class_feats].values; y = clf_df["elite"].values
            if HAVE_SKLEARN:
                ...
                proba_lg = logit.predict_proba(X_test)[:,1]; proba_rf = rfc.predict_proba(X_test)[:,1]
                ...
                m_lg = clf_metrics(y_test, proba_lg); m_rf = clf_metrics(y_test, proba_rf)
                ...
                with col1:
                    ...
                    cv_auc = cross_val_score(logit, X, y, cv=skf, scoring="roc_auc")
                ...
                with col2:
                    ...
                    skf = StratifiedKFold(n_splits=cv_k, shuffle=True, random_state=seed)
                    cv_auc = cross_val_score(rfc, X, y, cv=skf, scoring="roc_auc", n_jobs=-1)
                    st.caption(f"CV ROC AUC mean={cv_auc.mean():.3f} ± {cv_auc.std():.3f}")
                fpr_lg, tpr_lg, _ = roc_curve(y_test, proba_lg); fpr_rf, tpr_rf, _ = roc_curve(y_test, proba_rf)
                fig, ax = plt.subplots(figsize=(6,5))
                ax.plot(fpr_lg, tpr_lg, label=f"LogReg (AUC={m_lg['ROC_AUC']:.3f})")
                ax.plot(fpr_rf, tpr_rf, label=f"RF (AUC={m_rf['ROC_AUC']:.3f})")
                ax.plot([0,1],[0,1],"k--", alpha=0.5); ax.set_xlabel("FPR"); ax.set_ylabel("TPR"); ax.set_title("ROC Curve"); ax.legend()
                st.pyplot(fig, use_container_width=True)
                ...
                # NumPy fallback: Logistic + GaussianNB
                n = len(X); idx = np.random.default_rng(seed).permutation(n)
                n_test = max(1, int(test_size*n)); test_idx = idx[:n_test]; train_idx = idx[n_test:]
                X_train, X_test, y_train, y_test = X[train_idx], X[test_idx], y[train_idx], y[test_idx]
                lg = np_logistic_train(X_train, y_train, lr=0.1, epochs=2000, l2=1e-3)
                proba_lg = np_logistic_proba(lg, X_test)
                gnb = GaussianNB().fit(X_train, y_train); proba_nb = gnb.predict_proba(X_test)[:,1]
                def clf_metrics_simple(y_true, proba):
                    pred = (proba>=0.5).astype(int)
                    return {"Accuracy": accuracy_score(y_true,pred),
                            "Precision": precision_score(y_true,pred, zero_division=0),
                            "Recall": recall_score(y_true,pred, zero_division=0),
                            "F1": f1_score(y_true,pred, zero_division=0),
                            "ROC_AUC": roc_auc_score(y_true,proba)}
                m_lg = clf_metrics_simple(y_test, proba_lg); m_nb = clf_metrics_simple(y_test, proba_nb)
                col1,col2 = st.columns(2)
                with col1: st.markdown("Logistic (NumPy)"); st.json({k: round(v,3) for k,v in m_lg.items()})
                with col2: st.markdown("Gaussian NB (NumPy)"); st.json({k: round(v,3) for k,v in m_nb.items()})
                pred_df = pd.DataFrame({"y_true": y_test, "proba_logreg": proba_lg, "proba_gnb": proba_nb})
                st.download_button("Download classification predictions (NumPy)", data=to_csv_bytes(pred_df), file_name="classification_predictions_numpy.csv", mime="text/csv")

# --- HoF Explorer ---
with tabs[5]:
    st.subheader("Hall of Fame Index Explorer (0-100)")
    st.markdown("Weights: MVP(15), Finals MVP(12), Rings(8), DPOY(5), All-NBA 1st(4), All-NBA Total(2.5), All-Star(2), All-Def 1st(2), All-Def Total(1), ROY(1.5), Scoring(1.5).")
    if career_df.empty:
        ...
        with col1: min_seasons = safe_int_slider("Minimum seasons", 0, max_s, 0, step=1, key="min_seasons")
        with col2: min_games   = safe_int_slider("Minimum games",   0, max_g, 0, step=(50 if max_g>=50 else 1), key="min_games")
        filt = career_df.copy()
            ...
            if players:
                ...
                if not row.empty:
                    ...
                    with c:
                        ...
                        if "avg_team_win_pct" in r.index and pd.notna(r["avg_team_win_pct"]): st.write(f"Avg Win%: {r['avg_team_win_pct']:.3f}")
                    if "hof_index" in r.index:
                        hof_idx=float(r["hof_index"]); st.markdown(f"HoF Index: **{hof_idx:.1f} / 100**")
                        fig, ax = plt.subplots(figsize=(8,1.6))
                        ax.barh([0],[hof_idx], height=0.5); ax.set_xlim(0,100); ax.set_yticks([]); ax.set_xlabel("HoF Index (0-100)")
                        ax.axvline(50,color="gray",linestyle="--",alpha=0.3,label="Avg")
                        ax.axvline(85,color="blue",linestyle="--",alpha=0.3,label="Strong")
                        ax.axvline(95,color="green",linestyle="--",alpha=0.3,label="Elite")
                        ...
                        if contrib_cols:
                            ...
                            topk_max=max(5, min(20, len(contrib))); topk = safe_int_slider("Show top K contributors", 5, topk_max, min(10, topk_max), step=1, key="topk")
                            cc = contrib.head(topk); fig, ax = plt.subplots(figsize=(8,4))
                            cc.plot(kind="barh", ax=ax); ax.invert_yaxis(); ax.set_title("Top HoF feature contributions (std-weighted)")
                            st.pyplot(fig, use_container_width=True)
        st.markdown("---")
        st.download_button("⬇️ Download career table with accolades & HoF Index",
                           data=to_csv_bytes(career_df),
                           file_name="career_with_accolades_hof_index.csv", mime="text/csv")
